        ...
        if ((motor->motor_type==M3508)&&(motor->can_dev == chassis_can))
        {
            switch (motor->rx_id) {
                ...
                case 0x204:
                {
                    ...
                    power_limit_test = struct trans_fdb_msg->robot_status.chassis_power_limit;
...
struct subscriber_t *sub_trans;
static struct trans_fdb_msg  trans_fdb;

static void trans_fdb_msg_init( )
{
    sub_trans= sub_register("trans_fdb", sizeof(struct trans_fdb_msg));

}

static void trans_fdb_msg_subscribe( struct trans_fdb_msg *msg )
{
